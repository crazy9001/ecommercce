...
var sort_type = 'time';
var multi_selection_enabled = (getUrlParam('multi')!=='true'?false:true);
var selected = [];
var items = [];
...
$(document).on('click', '#content a', function (e) {
  ...
  if (multi_selection_enabled) {
    ...
    var item = getOneSelectedElement(element.data('id'));
    if (item.is_file) {
      use(item);
    } else {
      goTo(item.url);
...
function preview(item) {
    $('#priview').find('.btn-primary').toggle(true);
    $('#priview').find('.btn-primary').unbind().click(undefined);
    $('#priview').modal('show').find('.modal-body').html($('<img>').addClass('w-100').attr('src', item.url + '?timestamp=' + item.time));
}
